...
d = 0.01; % Diameter of Shaft % m
J = (pi / 32) * d^4; %m^4 % Polar Moment of Inertia of Area of Shaft with Hole
            % Using parallel axis theorem

L = [0.05, 0.075, 0.055]; % Length of Shaft Elements % m

Ip = [0.002, 0.001, 0.008]; % Polar Mass Moment of Inertia of the Disc
...
for i = 1:nele+1
    coord(i,:) = [i, sum(L(1:i-1))];
end

%% Boundary Conditions

bc = [3]; % for every elements
...
for i = 1:nele
    ...
    if(bc(p) == 3)
        m_ele(:,:,i) = [0, 0; 0, Ip(nd1)] + (rho * J * l / 6) * [2, 1; 1, 2]; % Disc at the left free end
    else
